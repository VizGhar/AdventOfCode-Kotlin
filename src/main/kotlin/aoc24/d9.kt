...
object Day9 {
    ...
    private val input = File("inputs/aoc24/d9.txt").readLines().let {
        val gaps = mutableListOf<Block>()
        val blocks = mutableListOf<Block>()
        it[0].forEachIndexed { index, c ->
            Block(
                id = index / 2,
                index = it[0].substring(0, index).sumOf { it.digitToInt() },
                size = c.digitToInt()
            ).let {
    ...
    fun part1(): Long {
        val gaps = input.gaps.toMutableList()
        val blocks = input.blocks.toMutableList()
        for (blockId in input.blocks.size - 1 downTo 0) {
            val block = input.blocks[blockId]
