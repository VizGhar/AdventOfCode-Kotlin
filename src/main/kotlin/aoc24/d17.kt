...
object Day17 {
    ...
    private val input = File("inputs/aoc24/d17.txt").readLines().let{
        Input(
            registerA = it[0].substringAfter(": ").toLong(),
            registerB = it[1].substringAfter(": ").toLong(),
            registerC = it[2].substringAfter(": ").toLong(),
            program = it[4].substringAfter(": ").split(",").map { it.toInt() }
        )
    }
                ...
                5 -> {
                    ...
                    if (drop) {
                        ...
                        for (i in 0..<result.size) {
                            if (i >= input.program.size || result[i] != input.program[i]) matching = false
                        }
    ...
    fun part1() = runOver(input.copy(), false).joinToString(",")
    ...
    fun part2(): Long {
        ...
        while (true) {
            ...
            for (p in patterns) {
                ...
                val out = runOver(input.copy(registerA = iteration), true)

                var matching = true
                for (a in out.indices) {
                    if (a >= input.program.size || out[a] != input.program[a]) matching = false
                }
                if (out == input.program) return iteration
